...
function build_acl()
{
    ...
    do
        if [ "$1"x = "func"x ]; then
            ...
            target_type="board"
            tool_chain=aarch64-v01c01-linux-musl-g++
        elif [ "$1"x = "musl"x ]; then
            echo "toolchain is aarch64-v01c01-linux-musl-g++"
            target_type="board"
            tool_chain=aarch64-v01c01-linux-musl-g++
        elif [ "$1"x = "glibc"x ]; then
            echo "toolchain is aarch64-v01c01-linux-gnu-g++"
            target_type="board"
            tool_chain=aarch64-v01c01-linux-gnu-g++
        else
...
SWITCH_CROSS_DIR=$DDK_PATH/acllib
cd $SWITCH_CROSS_DIR
ORI_CROSS_COMPILE_CHAIN=$(readlink lib64)
ORI_CROSS_COMPILE_CHAIN=${ORI_CROSS_COMPILE_CHAIN#*_}
echo "ORI_CROSS_COMPILE_CHAIN=$ORI_CROSS_COMPILE_CHAIN"
cd $CUR_DIR
...
#switch back to the initial cross-compilation chain
cd $SWITCH_CROSS_LIB_DIR
bash $SWITCH_CROSS_LIB_DIR/switch_cross_lib.sh $ORI_CROSS_COMPILE_CHAIN
